...
async function getTokens(network: NetworkBase) {
  ...
  return Object.values(data)
    ...
    .filter(
      (token) =>
        token.chainInfo.chainType === network.chainType &&
        token.chainInfo.chainId === network.chainId
    );
}
...
export default async function NetworkPage({ params }: NetworkPageProps) {
  const network = {
    chainType: params.slug[0] as NetworkType,
    chainId: Number(params.slug[1]),
  };
  const tokens = await getTokens(network);

  return (
    <main className="flex min-h-screen flex-col items-center p-24">
      <div className="max-w-5xl w-full items-center justify-between text-sm lg:flex px-5 py-4">
        <a
          href="../"
      ...
      <div className="text-center max-w-5xl w-full px-5">
  ...
  return (
    <div className="group rounded-lg border border-transparent px-5 py-4 transition-colors hover:border-gray-300 hover:bg-gray-100 hover:dark:border-neutral-700 hover:dark:bg-neutral-800/30">
      ...
      <h4 className="text-sm font-mono">
        {shortenHash(token.contractAddress)}
      </h4>
